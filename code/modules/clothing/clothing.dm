...
/proc/generate_female_clothing(index, t_color, icon, type, flat = FALSE)	//MONKESTATION EDIT - Flat lizards
	var/icon/female_clothing_icon = icon("icon"=icon, "icon_state"=t_color)
	var/female_icon_state = "female[type == FEMALE_UNIFORM_FULL ? "_full" : ((!type || type & FEMALE_UNIFORM_TOP_ONLY) ? "_top" : "")][((type & FEMALE_UNIFORM_NO_BREASTS) || flat) ? "_no_breasts" : ""]"
